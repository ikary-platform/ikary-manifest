...
Textarea provides the canonical implementation unit for the `textarea` contract in this repository. It converts validated presentation payloads into deterministic runtime rendering through the standard pipeline: schema, validator, adapter, resolver, and register. This LLD constrains implementation choices so code generation stays consistent with existing `cell-contract-presentation` and `cell-runtime-ui` patterns.

## 2. Scope

- Normalize and validate presentation shape defined by `TextareaPresentationSchema`.
- Expose a stable runtime contract for `Textarea` rendering.
...
- Own canonical fields such as `value`, `defaultValue`, `placeholder`, `rows`, `disabled`, `readonly`.
...
- Presentation input: `TextareaPresentation` validated by the runtime validator in this folder.
...
- Primary schema fields consumed at runtime include `value`, `defaultValue`, `placeholder`, `rows`, `disabled`, `readonly`, `required`, `invalid`.
...
- Contract schema: `libs/cell-contract-presentation/src/contract/textarea/TextareaPresentationSchema.ts`.
- Contract validator: `libs/cell-contract-presentation/src/contract/textarea/validate-runtime-textarea-presentation.ts`.
- LLD doc: `libs/cell-contract-presentation/src/contract/textarea/Textarea.LLD.md`.
- Runtime: `libs/cell-runtime-ui/src/primitives/textarea/Textarea.adapter.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/textarea/Textarea.resolver.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/textarea/Textarea.tsx`.
- Runtime: `libs/cell-runtime-ui/src/primitives/textarea/Textarea.types.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/textarea/index.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/textarea/registerTextarea.ts`.
