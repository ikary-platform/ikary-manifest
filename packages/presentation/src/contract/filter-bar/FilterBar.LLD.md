...
FilterBar provides the canonical implementation unit for the `filter-bar` contract in this repository. It converts validated presentation payloads into deterministic runtime rendering through the standard pipeline: schema, validator, adapter, resolver, and register. This LLD constrains implementation choices so code generation stays consistent with existing `cell-contract-presentation` and `cell-runtime-ui` patterns.

## 2. Scope

- Normalize and validate presentation shape defined by `FilterBarPresentationSchema`.
- Expose a stable runtime contract for `FilterBar` rendering.
...
- Own canonical fields such as `value`, `label`, `placeholder`, `disabled`, `key`, `type`.
...
- Presentation input: `FilterBarPresentation` validated by the runtime validator in this folder.
...
- Primary schema fields consumed at runtime include `value`, `label`, `placeholder`, `disabled`, `key`, `type`, `options`, `code`.
...
- Contract schema: `libs/cell-contract-presentation/src/contract/filter-bar/FilterBarPresentationSchema.ts`.
- Contract validator: `libs/cell-contract-presentation/src/contract/filter-bar/validate-runtime-filter-bar-presentation.ts`.
- LLD doc: `libs/cell-contract-presentation/src/contract/filter-bar/FilterBar.LLD.md`.
- Runtime: `libs/cell-runtime-ui/src/primitives/filter-bar/FilterBar.adapter.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/filter-bar/FilterBar.resolver.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/filter-bar/FilterBar.tsx`.
- Runtime: `libs/cell-runtime-ui/src/primitives/filter-bar/FilterBar.types.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/filter-bar/index.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/filter-bar/registerFilterBar.ts`.
