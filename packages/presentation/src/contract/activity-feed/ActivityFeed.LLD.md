...
ActivityFeed provides the canonical implementation unit for the `activity-feed` contract in this repository. It converts validated presentation payloads into deterministic runtime rendering through the standard pipeline: schema, validator, adapter, resolver, and register. This LLD constrains implementation choices so code generation stays consistent with existing `cell-contract-presentation` and `cell-runtime-ui` patterns.

## 2. Scope

- Normalize and validate presentation shape defined by `ActivityFeedPresentationSchema`.
- Expose a stable runtime contract for `ActivityFeed` rendering.
...
- Own canonical fields such as `label`, `href`, `actionKey`, `code`, `path`, `message`.
...
- Presentation input: `ActivityFeedPresentation` validated by the runtime validator in this folder.
...
- Primary schema fields consumed at runtime include `label`, `href`, `actionKey`, `code`, `path`, `message`, `key`, `summary`.
...
- Treat `href` as declarative navigation metadata; avoid direct router mutation in component code.
...
- Contract schema: `libs/cell-contract-presentation/src/contract/activity-feed/ActivityFeedPresentationSchema.ts`.
- Contract validator: `libs/cell-contract-presentation/src/contract/activity-feed/validate-runtime-activity-feed-presentation.ts`.
- LLD doc: `libs/cell-contract-presentation/src/contract/activity-feed/ActivityFeed.LLD.md`.
- Runtime: `libs/cell-runtime-ui/src/primitives/activity-feed/ActivityFeed.adapter.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/activity-feed/ActivityFeed.resolver.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/activity-feed/ActivityFeed.tsx`.
- Runtime: `libs/cell-runtime-ui/src/primitives/activity-feed/ActivityFeed.types.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/activity-feed/index.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/activity-feed/registerActivityFeed.ts`.
