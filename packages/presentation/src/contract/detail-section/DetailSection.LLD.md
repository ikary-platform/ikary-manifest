...
DetailSection provides the canonical implementation unit for the `detail-section` contract in this repository. It converts validated presentation payloads into deterministic runtime rendering through the standard pipeline: schema, validator, adapter, resolver, and register. This LLD constrains implementation choices so code generation stays consistent with existing `cell-contract-presentation` and `cell-runtime-ui` patterns.

## 2. Scope

- Normalize and validate presentation shape defined by `DetailSectionPresentationSchema`.
- Expose a stable runtime contract for `DetailSection` rendering.
...
- Own canonical fields such as `key`, `label`, `actionKey`, `href`, `intent`, `icon`.
...
- Presentation input: `DetailSectionPresentation` validated by the runtime validator in this folder.
...
- Primary schema fields consumed at runtime include `key`, `label`, `actionKey`, `href`, `intent`, `icon`, `disabled`, `hiddenWhenUnauthorized`.
...
- Contract schema: `libs/cell-contract-presentation/src/contract/detail-section/DetailSectionPresentationSchema.ts`.
- LLD doc: `libs/cell-contract-presentation/src/contract/detail-section/DetailSection.LLD.md`.
- Runtime: `libs/cell-runtime-ui/src/primitives/detail-section/DetailSection.adapter.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/detail-section/DetailSection.register.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/detail-section/DetailSection.resolver.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/detail-section/DetailSection.tsx`.
- Runtime: `libs/cell-runtime-ui/src/primitives/detail-section/DetailSection.types.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/detail-section/index.ts`.
