...
LoadingState provides the canonical implementation unit for the `loading-state` contract in this repository. It converts validated presentation payloads into deterministic runtime rendering through the standard pipeline: schema, validator, adapter, resolver, and register. This LLD constrains implementation choices so code generation stays consistent with existing `cell-contract-presentation` and `cell-runtime-ui` patterns.

## 2. Scope

- Normalize and validate presentation shape defined by `LoadingStatePresentationSchema`.
- Expose a stable runtime contract for `LoadingState` rendering.
...
- Own canonical fields such as `lines`, `blocks`, `avatar`, `variant`, `density`, `mode`.
...
- Presentation input: `LoadingStatePresentation` validated by the runtime validator in this folder.
...
- Primary schema fields consumed at runtime include `lines`, `blocks`, `avatar`, `variant`, `density`, `mode`, `label`, `description`.
...
- Contract schema: `libs/cell-contract-presentation/src/contract/loading-state/LoadingStatePresentationSchema.ts`.
- Contract validator: `libs/cell-contract-presentation/src/contract/loading-state/validate-runtime-loading-state-presentation.ts`.
- LLD doc: `libs/cell-contract-presentation/src/contract/loading-state/LoadingState.LLD.md`.
- Runtime: `libs/cell-runtime-ui/src/primitives/loading-state/LoadingState.adapter.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/loading-state/LoadingState.resolver.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/loading-state/LoadingState.tsx`.
- Runtime: `libs/cell-runtime-ui/src/primitives/loading-state/LoadingState.types.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/loading-state/index.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/loading-state/registerLoadingState.ts`.
