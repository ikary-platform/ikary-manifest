...
ErrorState provides the canonical implementation unit for the `error-state` contract in this repository. It converts validated presentation payloads into deterministic runtime rendering through the standard pipeline: schema, validator, adapter, resolver, and register. This LLD constrains implementation choices so code generation stays consistent with existing `cell-contract-presentation` and `cell-runtime-ui` patterns.

## 2. Scope

- Normalize and validate presentation shape defined by `ErrorStatePresentationSchema`.
- Expose a stable runtime contract for `ErrorState` rendering.
...
- Own canonical fields such as `label`, `actionKey`, `href`, `code`, `path`, `message`.
...
- Presentation input: `ErrorStatePresentation` validated by the runtime validator in this folder.
...
- Primary schema fields consumed at runtime include `label`, `actionKey`, `href`, `code`, `path`, `message`, `correlationId`, `title`.
...
- Contract schema: `libs/cell-contract-presentation/src/contract/error-state/ErrorStatePresentationSchema.ts`.
- Contract validator: `libs/cell-contract-presentation/src/contract/error-state/validate-runtime-error-state-presentation.ts`.
- LLD doc: `libs/cell-contract-presentation/src/contract/error-state/ErrorState.LLD.md`.
- Runtime: `libs/cell-runtime-ui/src/primitives/error-state/ErrorState.adapter.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/error-state/ErrorState.register.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/error-state/ErrorState.resolver.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/error-state/ErrorState.tsx`.
- Runtime: `libs/cell-runtime-ui/src/primitives/error-state/ErrorState.types.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/error-state/index.ts`.
