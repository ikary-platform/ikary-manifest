# CrossEntityValidatorRefSchema LLD
...
- `ruleId` from the validated contract payload.
- `type` from the validated contract payload.
- `validatorRef` from the validated contract payload.
- `messageKey` from the validated contract payload.
- `clientSafe` from the validated contract payload.
- `async` from the validated contract payload.
- `blocking` from the validated contract payload.
- `severity` from the validated contract payload.
- Optional integration context from callers (never hardcoded in schema).

## 5. Primitive Composition

- Composes nested schemas: `ValidationSeveritySchema`.
...
- Schema source: `libs/cell-contract-core/src/contract/entity/CrossEntityValidatorRefSchema.ts`.
- Human contract doc: `CrossEntityValidatorRefSchema.md`.
- LLM contract doc: `CrossEntityValidatorRefSchema.llm.md`.
- Samples file: `CrossEntityValidatorRefSchema.samples.json` (minimum 5 valid examples).
