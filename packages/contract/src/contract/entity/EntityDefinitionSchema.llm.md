...
- `pluralName` from the validated contract payload.
- `fields` from the validated contract payload.
- Optional integration context from callers (never hardcoded in schema).

## 5. Primitive Composition

- Composes nested schemas: `CapabilityDefinitionSchema`, `ComputedFieldDefinitionSchema`, `EntityPoliciesDefinitionSchema`, `EntityValidationSchema`, `EventDefinitionSchema`, `FieldDefinitionSchema`, `FieldPoliciesDefinitionSchema`, `LifecycleDefinitionSchema`.
...
- Schema source: `libs/cell-contract-core/src/contract/entity/EntityDefinitionSchema.ts`.
- Human contract doc: `EntityDefinitionSchema.md`.
- LLM contract doc: `EntityDefinitionSchema.llm.md`.
- Samples file: `EntityDefinitionSchema.samples.json` (minimum 5 valid examples).
