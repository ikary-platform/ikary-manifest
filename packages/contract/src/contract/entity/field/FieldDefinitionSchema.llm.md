...
- `validation` from the validated contract payload.
- Optional integration context from callers (never hardcoded in schema).

## 5. Primitive Composition

- Composes nested schemas: `DisplayDefinitionSchema`, `FieldOperationMetaSchema`, `FieldTypeSchema`, `FieldValidationSchema`.
...
- Schema source: `libs/cell-contract-core/src/contract/entity/field/FieldDefinitionSchema.ts`.
- Human contract doc: `FieldDefinitionSchema.md`.
- LLM contract doc: `FieldDefinitionSchema.llm.md`.
- Samples file: `FieldDefinitionSchema.samples.json` (minimum 5 valid examples).
