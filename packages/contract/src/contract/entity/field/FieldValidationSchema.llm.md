...
- `fieldRules` from the validated contract payload.
- Optional integration context from callers (never hardcoded in schema).

## 5. Primitive Composition

- Composes nested schemas: `FieldRuleDefinitionSchema`.
...
- Schema source: `libs/cell-contract-core/src/contract/entity/field/FieldValidationSchema.ts`.
- Human contract doc: `FieldValidationSchema.md`.
- LLM contract doc: `FieldValidationSchema.llm.md`.
- Samples file: `FieldValidationSchema.samples.json` (minimum 5 valid examples).
