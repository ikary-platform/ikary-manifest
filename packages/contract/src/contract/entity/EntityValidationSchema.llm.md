...
- `entityRules` from the validated contract payload.
- Optional integration context from callers (never hardcoded in schema).

## 5. Primitive Composition

- Composes nested schemas: `CrossEntityValidatorRefSchema`, `EntityRuleDefinitionSchema`.
...
- Schema source: `libs/cell-contract-core/src/contract/entity/EntityValidationSchema.ts`.
- Human contract doc: `EntityValidationSchema.md`.
- LLM contract doc: `EntityValidationSchema.llm.md`.
- Samples file: `EntityValidationSchema.samples.json` (minimum 5 valid examples).
