...
- `pageKey` from the validated contract payload.
- `label` from the validated contract payload.
- `order` from the validated contract payload.
...
- Schema source: `libs/cell-contract-core/src/contract/manifest/navigation/NavigationItemSchema.ts`.
- Human contract doc: `NavigationItemSchema.md`.
- LLM contract doc: `NavigationItemSchema.llm.md`.
- Samples file: `NavigationItemSchema.samples.json` (minimum 5 valid examples).
