...
- `layout` from the validated contract payload.
- `regions` from the validated contract payload.
- `outlet` from the validated contract payload.
- Optional integration context from callers (never hardcoded in schema).

## 5. Primitive Composition

- Composes nested schemas: `AppShellLayoutModeSchema`, `ShellCapabilitiesSchema`, `ShellNavItemDefinitionSchema`, `ShellNavigationDefinitionSchema`, `ShellRegionDefinitionSchema`, `ShellRegionKeySchema`, `ShellResponsiveDefinitionSchema`.
...
- Schema source: `libs/cell-contract-core/src/contract/manifest/shell/AppShellDefinitionSchema.ts`.
- Human contract doc: `AppShellDefinitionSchema.md`.
- LLM contract doc: `AppShellDefinitionSchema.llm.md`.
- Samples file: `AppShellDefinitionSchema.samples.json` (minimum 5 valid examples).
