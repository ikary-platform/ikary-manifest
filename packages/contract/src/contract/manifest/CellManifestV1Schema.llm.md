...
- `apiVersion` from the validated contract payload.
- `kind` from the validated contract payload.
- `metadata` from the validated contract payload.
- `spec` from the validated contract payload.
- Optional integration context from callers (never hardcoded in schema).

## 5. Primitive Composition

- Composes nested schemas: `CellMetadataSchema`, `CellSpecSchema`.
...
- Schema source: `libs/cell-contract-core/src/contract/manifest/CellManifestV1Schema.ts`.
- Human contract doc: `CellManifestV1Schema.md`.
- LLM contract doc: `CellManifestV1Schema.llm.md`.
- Samples file: `CellManifestV1Schema.samples.json` (minimum 5 valid examples).
