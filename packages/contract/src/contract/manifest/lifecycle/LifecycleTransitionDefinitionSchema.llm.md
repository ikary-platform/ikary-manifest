...
- `from` from the validated contract payload.
- `to` from the validated contract payload.
- Optional integration context from callers (never hardcoded in schema).

## 5. Primitive Composition

- Composes no child contract schemas directly; validation is self-contained.
- Integrates with nearby index exports in the same contract folder.
...
- Schema source: `libs/cell-contract-core/src/contract/manifest/lifecycle/LifecycleTransitionDefinitionSchema.ts`.
- Human contract doc: `LifecycleTransitionDefinitionSchema.md`.
- LLM contract doc: `LifecycleTransitionDefinitionSchema.llm.md`.
- Samples file: `LifecycleTransitionDefinitionSchema.samples.json` (minimum 5 valid examples).
