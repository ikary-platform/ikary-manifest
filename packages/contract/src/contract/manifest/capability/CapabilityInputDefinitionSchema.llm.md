...
- `type` from the validated contract payload.
- `label` from the validated contract payload.
- `defaultValue` from the validated contract payload.
- Optional integration context from callers (never hardcoded in schema).

## 5. Primitive Composition

- Composes nested schemas: `BooleanCapabilityInputSchema`, `CapabilityInputBaseSchema`, `CapabilityInputTypeSchema`, `DateCapabilityInputSchema`, `EntityCapabilityInputSchema`, `NumberCapabilityInputSchema`, `SelectCapabilityInputSchema`, `StringCapabilityInputSchema`.
...
- Schema source: `libs/cell-contract-core/src/contract/manifest/capability/CapabilityInputDefinitionSchema.ts`.
- Human contract doc: `CapabilityInputDefinitionSchema.md`.
- LLM contract doc: `CapabilityInputDefinitionSchema.llm.md`.
- Samples file: `CapabilityInputDefinitionSchema.samples.json` (minimum 5 valid examples).
