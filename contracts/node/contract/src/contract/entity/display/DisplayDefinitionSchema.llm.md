...
- `emptyLabel` from the validated contract payload.
- `truncate` from the validated contract payload.
- `align` from the validated contract payload.
- Optional integration context from callers (never hardcoded in schema).

## 5. Primitive Composition

- Composes nested schemas: `DisplayAlignSchema`, `DisplayTypeSchema`.
...
- Schema source: `libs/cell-contract-core/src/contract/entity/display/DisplayDefinitionSchema.ts`.
- Human contract doc: `DisplayDefinitionSchema.md`.
- LLM contract doc: `DisplayDefinitionSchema.llm.md`.
- Samples file: `DisplayDefinitionSchema.samples.json` (minimum 5 valid examples).
