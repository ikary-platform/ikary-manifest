...
- `event_id` from the validated contract payload.
- `event_name` from the validated contract payload.
- `version` from the validated contract payload.
- `timestamp` from the validated contract payload.
- `tenant_id` from the validated contract payload.
- `workspace_id` from the validated contract payload.
- `cell_id` from the validated contract payload.
- `actor` from the validated contract payload.
- Optional integration context from callers (never hardcoded in schema).

## 5. Primitive Composition

- Composes nested schemas: `DomainEventActorSchema`, `DomainEventEntityRefSchema`.
...
- Schema source: `libs/cell-contract-core/src/contract/manifest/domain-event/DomainEventEnvelopeSchema.ts`.
- Human contract doc: `DomainEventEnvelopeSchema.md`.
- LLM contract doc: `DomainEventEnvelopeSchema.llm.md`.
- Samples file: `DomainEventEnvelopeSchema.samples.json` (minimum 5 valid examples).
