# ComputedFieldDefinitionSchema LLD
...
- `name` from the validated contract payload.
- `type` from the validated contract payload.
- `formulaType` from the validated contract payload.
- `expression` from the validated contract payload.
- `dependencies` from the validated contract payload.
- Optional integration context from callers (never hardcoded in schema).

## 5. Primitive Composition

- Composes nested schemas: `AggregationComputedFieldSchema`, `AggregationOperationSchema`, `ComputedFieldBaseSchema`, `ComputedValueTypeSchema`, `ConditionalComputedFieldSchema`, `ExpressionComputedFieldSchema`.
...
- Schema source: `libs/cell-contract-core/src/contract/entity/field/ComputedFieldDefinitionSchema.ts`.
- Human contract doc: `ComputedFieldDefinitionSchema.md`.
- LLM contract doc: `ComputedFieldDefinitionSchema.llm.md`.
- Samples file: `ComputedFieldDefinitionSchema.samples.json` (minimum 5 valid examples).
