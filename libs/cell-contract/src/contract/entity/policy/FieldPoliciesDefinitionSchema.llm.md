...
- `keya_1` from the validated contract payload.
- Optional integration context from callers (never hardcoded in schema).

## 5. Primitive Composition

- Composes nested schemas: `ActionPolicySchema`, `FieldPolicyOverrideSchema`.
...
- Schema source: `libs/cell-contract-core/src/contract/entity/policy/FieldPoliciesDefinitionSchema.ts`.
- Human contract doc: `FieldPoliciesDefinitionSchema.md`.
- LLM contract doc: `FieldPoliciesDefinitionSchema.llm.md`.
- Samples file: `FieldPoliciesDefinitionSchema.samples.json` (minimum 5 valid examples).
