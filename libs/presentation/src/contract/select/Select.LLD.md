...
Select provides the canonical implementation unit for the `select` contract in this repository. It converts validated presentation payloads into deterministic runtime rendering through the standard pipeline: schema, validator, adapter, resolver, and register. This LLD constrains implementation choices so code generation stays consistent with existing `cell-contract-presentation` and `cell-runtime-ui` patterns.

## 2. Scope

- Normalize and validate presentation shape defined by `SelectPresentationSchema`.
- Expose a stable runtime contract for `Select` rendering.
...
- Own canonical fields such as `value`, `label`, `disabled`, `description`, `icon`, `defaultValue`.
...
- Presentation input: `SelectPresentation` validated by the runtime validator in this folder.
...
- Primary schema fields consumed at runtime include `value`, `label`, `disabled`, `description`, `icon`, `defaultValue`, `placeholder`, `required`.
...
- Contract schema: `libs/cell-contract-presentation/src/contract/select/SelectPresentationSchema.ts`.
- Contract validator: `libs/cell-contract-presentation/src/contract/select/validate-runtime-select-presentation.ts`.
- LLD doc: `libs/cell-contract-presentation/src/contract/select/Select.LLD.md`.
- Runtime: `libs/cell-runtime-ui/src/primitives/select/Select.adapter.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/select/Select.resolver.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/select/Select.tsx`.
- Runtime: `libs/cell-runtime-ui/src/primitives/select/Select.types.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/select/index.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/select/registerSelect.ts`.
