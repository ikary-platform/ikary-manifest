...
RadioGroup provides the canonical implementation unit for the `radio-group` contract in this repository. It converts validated presentation payloads into deterministic runtime rendering through the standard pipeline: schema, validator, adapter, resolver, and register. This LLD constrains implementation choices so code generation stays consistent with existing `cell-contract-presentation` and `cell-runtime-ui` patterns.

## 2. Scope

- Normalize and validate presentation shape defined by `RadioGroupPresentationSchema`.
- Expose a stable runtime contract for `RadioGroup` rendering.
...
- Own canonical fields such as `value`, `label`, `disabled`, `description`, `defaultValue`, `required`.
...
- Presentation input: `RadioGroupPresentation` validated by the runtime validator in this folder.
...
- Primary schema fields consumed at runtime include `value`, `label`, `disabled`, `description`, `defaultValue`, `required`, `invalid`, `loading`.
...
- Contract schema: `libs/cell-contract-presentation/src/contract/radio-group/RadioGroupPresentationSchema.ts`.
- Contract validator: `libs/cell-contract-presentation/src/contract/radio-group/validate-runtime-radio-group-presentation.ts`.
- LLD doc: `libs/cell-contract-presentation/src/contract/radio-group/RadioGroup.LLD.md`.
- Runtime: `libs/cell-runtime-ui/src/primitives/radio-group/RadioGroup.adapter.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/radio-group/RadioGroup.resolver.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/radio-group/RadioGroup.tsx`.
- Runtime: `libs/cell-runtime-ui/src/primitives/radio-group/RadioGroup.types.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/radio-group/index.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/radio-group/registerRadioGroup.ts`.
