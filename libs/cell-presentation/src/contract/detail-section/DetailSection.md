...
`DetailSection` is the canonical read-only grouping primitive used inside `DetailPage` and other approved read surfaces.

It is a presentation primitive.
It is not a page shell.
It is not an editing surface.
It does not own mutation lifecycle.
...
A read surface must not feel like:

- raw database output
- disabled form controls
- arbitrary field dumping
...
- section-level empty state presentation
...
Ad hoc read-only field grids at page level are forbidden.
...
DetailSection  
 ...
 ├── Body  
 │ └── ReadOnlyContent  
 └── Footer (optional, restricted)

Rules:

- header appears before body
- title is mandatory
- description is optional
- body is mandatory
- footer is optional and must remain secondary
...
## 7.3 Compact/headerless mode

Headerless compact mode is not part of the standard V1 contract.

If ever needed, it must be explicitly system-approved.
Default expectation: every `DetailSection` has a visible title.

---

# 8. Body Rules

The body contains grouped read-only information.

Allowed body content must use one approved content mode.
...
# 9. Approved Content Modes

## 9.1 field-list

Use for standard read-only label/value presentation in a single-column flow.
...
Use for structured read-only label/value presentation when side-by-side scanability is beneficial.

Use when:

- values are short enough
- pairing improves readability
- label/value rhythm remains balanced
...
Use for compact display of key numeric or summary values.

Use when:

- values are concise
- comparative scanning matters
- the data is summary-like rather than descriptive

Metric presentation must remain calm and enterprise-oriented.

## 9.4 callout

Use for emphasized read-only information such as:
...
Callout must remain secondary to the page shell and must not become decorative.

## 9.5 custom-block

`custom-block` is a restricted escape hatch.

Use only when:

- none of the approved canonical modes fit
- the content is still read-only
- the use case is explicitly approved
- the section remains consistent with overall detail-page structure

`custom-block` must not become the default way to bypass canonical presentation.

For V1, `custom-block` should be rare.

---

# 10. Approved Layout Variants
...
# 11. Read-Only Value Presentation Rules
...
Footer is optional and restricted.

Use footer only when the section needs a clear secondary closure area, such as:

- small supporting action link
- extra explanatory note
- related navigation hint

Footer must:

- remain visually secondary to body content
- not become a second header
- not behave like a page-level action bar
- not carry primary mutation actions

If no meaningful footer exists, do not render one.

---

# 16. Collapsible Behavior
...
# 17. Status and Badge Presentation
...
# 18. Relationship and List Values
...
- use skeletons or placeholder rows inside the body
...
# 22. Layout and Spacing Rules

Section spacing must be consistent across the page.

Rules:

- header and body use approved spacing tokens
...
- using `custom-block` as the default presentation mode
...
## 25.4 Relationship-oriented section
...
## 25.5 Summary metric section

Title:
`Operational Summary`

Description:
`Key summary values for the current state of this entity.`
