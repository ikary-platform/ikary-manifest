...
FormField provides the canonical implementation unit for the `form-field` contract in this repository. It converts validated presentation payloads into deterministic runtime rendering through the standard pipeline: schema, validator, adapter, resolver, and register. This LLD constrains implementation choices so code generation stays consistent with existing `cell-contract-presentation` and `cell-runtime-ui` patterns.

## 2. Scope

- Normalize and validate presentation shape defined by `FormFieldPresentationSchema`.
- Expose a stable runtime contract for `FormField` rendering.
...
- Own canonical fields such as `key`, `label`, `value`, `description`, `disabled`, `tone`.
...
- Presentation input: `FormFieldPresentation` validated by the runtime validator in this folder.
...
- Primary schema fields consumed at runtime include `key`, `label`, `value`, `description`, `disabled`, `tone`, `text`, `type`.
...
- Direct primitive dependencies in component composition: `Checkbox`, `DateInput`, `Input`, `RadioGroup`, `Select`, `Textarea`, `Toggle`.
...
- Contract schema: `libs/cell-contract-presentation/src/contract/form-field/FormFieldPresentationSchema.ts`.
- LLD doc: `libs/cell-contract-presentation/src/contract/form-field/FormField.LLD.md`.
- Runtime: `libs/cell-runtime-ui/src/primitives/form-field/FormField.adapter.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/form-field/FormField.register.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/form-field/FormField.tsx`.
- Runtime: `libs/cell-runtime-ui/src/primitives/form-field/FormField.types.ts`.
