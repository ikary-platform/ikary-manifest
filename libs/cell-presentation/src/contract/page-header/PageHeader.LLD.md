...
PageHeader provides the canonical implementation unit for the `page-header` contract in this repository. It converts validated presentation payloads into deterministic runtime rendering through the standard pipeline: schema, validator, adapter, resolver, and register. This LLD constrains implementation choices so code generation stays consistent with existing `cell-contract-presentation` and `cell-runtime-ui` patterns.

## 2. Scope

- Normalize and validate presentation shape defined by `PageHeaderPresentationSchema`.
- Expose a stable runtime contract for `PageHeader` rendering.
...
- Own canonical fields such as `key`, `label`, `href`, `type`, `tone`, `actionKey`.
...
- Presentation input: `PageHeaderPresentation` validated by the runtime validator in this folder.
...
- Primary schema fields consumed at runtime include `key`, `label`, `href`, `type`, `tone`, `actionKey`, `intent`, `icon`.
...
- Direct primitive dependencies in component composition: `FieldValue`.
...
- Contract schema: `libs/cell-contract-presentation/src/contract/page-header/PageHeaderPresentationSchema.ts`.
- LLD doc: `libs/cell-contract-presentation/src/contract/page-header/PageHeader.LLD.md`.
- Runtime: `libs/cell-runtime-ui/src/primitives/page-header/PageHeader.adapter.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/page-header/PageHeader.register.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/page-header/PageHeader.resolver.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/page-header/PageHeader.tsx`.
- Runtime: `libs/cell-runtime-ui/src/primitives/page-header/PageHeader.types.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/page-header/index.ts`.
