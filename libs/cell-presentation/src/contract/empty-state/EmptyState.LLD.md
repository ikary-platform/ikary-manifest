...
EmptyState provides the canonical implementation unit for the `empty-state` contract in this repository. It converts validated presentation payloads into deterministic runtime rendering through the standard pipeline: schema, validator, adapter, resolver, and register. This LLD constrains implementation choices so code generation stays consistent with existing `cell-contract-presentation` and `cell-runtime-ui` patterns.

## 2. Scope

- Normalize and validate presentation shape defined by `EmptyStatePresentationSchema`.
- Expose a stable runtime contract for `EmptyState` rendering.
...
- Presentation input: `EmptyStatePresentation` validated by the runtime validator in this folder.
...
- Primary schema fields consumed at runtime include `label`, `actionKey`, `href`, `code`, `path`, `message`, `title`, `description`.
...
- Contract schema: `libs/cell-contract-presentation/src/contract/empty-state/EmptyStatePresentationSchema.ts`.
- Contract validator: `libs/cell-contract-presentation/src/contract/empty-state/validate-runtime-empty-state-presentation.ts`.
- LLD doc: `libs/cell-contract-presentation/src/contract/empty-state/EmptyState.LLD.md`.
- Runtime: `libs/cell-runtime-ui/src/primitives/empty-state/EmptyState.adapter.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/empty-state/EmptyState.resolver.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/empty-state/EmptyState.tsx`.
- Runtime: `libs/cell-runtime-ui/src/primitives/empty-state/EmptyState.types.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/empty-state/index.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/empty-state/registerEmptyState.ts`.
