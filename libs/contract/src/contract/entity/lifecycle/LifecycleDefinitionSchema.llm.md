...
- `initial` from the validated contract payload.
- `states` from the validated contract payload.
- `transitions` from the validated contract payload.
- Optional integration context from callers (never hardcoded in schema).

## 5. Primitive Composition

- Composes nested schemas: `LifecycleTransitionDefinitionSchema`.
...
- Schema source: `libs/cell-contract-core/src/contract/entity/lifecycle/LifecycleDefinitionSchema.ts`.
- Human contract doc: `LifecycleDefinitionSchema.md`.
- LLM contract doc: `LifecycleDefinitionSchema.llm.md`.
- Samples file: `LifecycleDefinitionSchema.samples.json` (minimum 5 valid examples).
