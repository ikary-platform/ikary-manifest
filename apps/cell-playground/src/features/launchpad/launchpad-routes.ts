...
export type LaunchpadTab = 'views' | 'data' | 'runtime' | 'primitives' | 'schemas';
...
const LOOKUP_BY_TAB: Record<LaunchpadTab, Map<string, LaunchpadBlockDefinition>> = {
  ...
  primitives: toBlockLookup('primitives'),
...
const BLOCKS_BY_TAB: Record<LaunchpadTab, LaunchpadBlockDefinition[]> = {
  ...
  primitives: BLOCKS.filter((block) => block.tab === 'primitives'),
...
const GROUPS_BY_TAB: Record<LaunchpadTab, LaunchpadTileGroupDefinition[]> = {
  ...
  primitives: toGroupTiles('primitives'),
...
export function getRuntimePathForPrimitive(primitive: string): string {
  const block = RUNTIME_BLOCKS_BY_PRIMITIVE.get(primitive);
  if (!block) {
    return toTabPath('primitives');
  }

  return toBlockPath('primitives', block.slug);
}
