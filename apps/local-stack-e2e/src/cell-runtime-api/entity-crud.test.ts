...
describe('cell-runtime-api — entity CRUD', () => {
  ...
  describe('GET list and GET by id', () => {
    ...
    it('GET by id returns 200 with matching record', async () => {
      const res = await fetch(`${ITEMS_URL}/${createdId}`);
      expect(res.status).toBe(200);
      const body = await res.json() as Record<string, unknown>;
  ...
  describe('DELETE removes a record', () => {
    ...
    it('DELETE returns 204', async () => {
      const res = await fetch(`${ITEMS_URL}/${recordId}`, { method: 'DELETE' });
  ...
  describe('pagination and sort', () => {
    ...
    it('page=1&limit=1 returns one record', async () => {
      const res = await fetch(`${ITEMS_URL}?page=1&limit=1&sort=count&order=asc`);
      expect(res.status).toBe(200);
    ...
    it('page=2&limit=1 returns a different record than page=1', async () => {
      const res1 = await fetch(`${ITEMS_URL}?page=1&limit=1&sort=count&order=asc`);
      const res2 = await fetch(`${ITEMS_URL}?page=2&limit=1&sort=count&order=asc`);
    ...
    it('sort by name asc returns records in ascending name order', async () => {
      const res = await fetch(`${ITEMS_URL}?sort=name&order=asc`);
