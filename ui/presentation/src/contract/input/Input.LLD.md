...
Input provides the canonical implementation unit for the `input` contract in this repository. It converts validated presentation payloads into deterministic runtime rendering through the standard pipeline: schema, validator, adapter, resolver, and register. This LLD constrains implementation choices so code generation stays consistent with existing `cell-contract-presentation` and `cell-runtime-ui` patterns.

## 2. Scope

- Normalize and validate presentation shape defined by `InputPresentationSchema`.
- Expose a stable runtime contract for `Input` rendering.
...
- Own canonical fields such as `inputType`, `value`, `defaultValue`, `placeholder`, `disabled`, `readonly`.
...
- Presentation input: `InputPresentation` validated by the runtime validator in this folder.
...
- Primary schema fields consumed at runtime include `inputType`, `value`, `defaultValue`, `placeholder`, `disabled`, `readonly`, `required`, `invalid`.
...
- Contract schema: `libs/cell-contract-presentation/src/contract/input/InputPresentationSchema.ts`.
- Contract validator: `libs/cell-contract-presentation/src/contract/input/validate-runtime-input-presentation.ts`.
- LLD doc: `libs/cell-contract-presentation/src/contract/input/Input.LLD.md`.
- Runtime: `libs/cell-runtime-ui/src/primitives/input/Input.adapter.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/input/Input.resolver.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/input/Input.tsx`.
- Runtime: `libs/cell-runtime-ui/src/primitives/input/Input.types.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/input/index.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/input/registerInput.ts`.
