...
Two additional gaps existed: no GitHub Release was created on publish, and no permanent branch was tagged per version, making it hard to reference or hotfix a specific release.
...
| `.github/workflows/release.yml` | Push to main | Detects changeset files, creates the Version Packages PR, and on that PR's merge: publishes to npm, creates a GitHub Release, and creates a `release/vX.Y.Z` branch. |
