# CapabilityDefinitionSchema LLD
...
- `transition` from the validated contract payload.
- `visible` from the validated contract payload.
- Optional integration context from callers (never hardcoded in schema).

## 5. Primitive Composition

- Composes nested schemas: `CapabilityBaseSchema`, `CapabilityInputDefinitionSchema`, `CapabilityScopeSchema`.
...
- Schema source: `libs/cell-contract-core/src/contract/manifest/capability/CapabilityDefinitionSchema.ts`.
- Human contract doc: `CapabilityDefinitionSchema.md`.
- LLM contract doc: `CapabilityDefinitionSchema.llm.md`.
- Samples file: `CapabilityDefinitionSchema.samples.json` (minimum 5 valid examples).
