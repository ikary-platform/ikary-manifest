...
- `mount` from the validated contract payload.
- `entities` from the validated contract payload.
- `pages` from the validated contract payload.
- Optional integration context from callers (never hardcoded in schema).

## 5. Primitive Composition

- Composes nested schemas: `AppShellDefinitionSchema`, `CellMountDefinitionSchema`, `EntityDefinitionSchema`, `NavigationDefinitionSchema`, `PageDefinitionSchema`, `RoleDefinitionSchema`.
...
- Schema source: `libs/cell-contract-core/src/contract/manifest/CellSpecSchema.ts`.
- Human contract doc: `CellSpecSchema.md`.
- LLM contract doc: `CellSpecSchema.llm.md`.
- Samples file: `CellSpecSchema.samples.json` (minimum 5 valid examples).
