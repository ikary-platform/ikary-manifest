...
FormSection provides the canonical implementation unit for the `form-section` contract in this repository. It converts validated presentation payloads into deterministic runtime rendering through the standard pipeline: schema, validator, adapter, resolver, and register. This LLD constrains implementation choices so code generation stays consistent with existing `cell-contract-presentation` and `cell-runtime-ui` patterns.

## 2. Scope

- Normalize and validate presentation shape defined by `FormSectionPresentationSchema`.
- Expose a stable runtime contract for `FormSection` rendering.
...
- Presentation input: `FormSectionPresentation` validated by the runtime validator in this folder.
...
- Direct primitive dependencies in component composition: `FormField`.
...
- Contract schema: `libs/cell-contract-presentation/src/contract/form-section/FormSectionPresentationSchema.ts`.
- LLD doc: `libs/cell-contract-presentation/src/contract/form-section/FormSection.LLD.md`.
- Runtime: `libs/cell-runtime-ui/src/primitives/form-section/FormSection.adapter.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/form-section/FormSection.register.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/form-section/FormSection.tsx`.
- Runtime: `libs/cell-runtime-ui/src/primitives/form-section/FormSection.types.ts`.
