...
CardList provides the canonical implementation unit for the `cardList` contract in this repository. It converts validated presentation payloads into deterministic runtime rendering through the standard pipeline: schema, validator, adapter, resolver, and register. This LLD constrains implementation choices so code generation stays consistent with existing `cell-contract-presentation` and `cell-runtime-ui` patterns.

## 2. Scope

- Normalize and validate presentation shape defined by `CardListPresentation`.
- Expose a stable runtime contract for `CardList` rendering.
...
- Own canonical fields such as `title`, `description`, `key`, `label`, `actionKey`, `href`.
...
- Presentation input: `CardListPresentation` validated by the runtime validator in this folder.
...
- Primary schema fields consumed at runtime include `title`, `description`, `key`, `label`, `actionKey`, `href`, `intent`, `icon`.
...
- Contract schema: `libs/cell-contract-presentation/src/contract/cardList/CardListPresentation.ts`.
- LLD doc: `libs/cell-contract-presentation/src/contract/cardList/CardList.LLD.md`.
- Runtime: `libs/cell-runtime-ui/src/primitives/card-list/CardList.adapter.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/card-list/CardList.register.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/card-list/CardList.resolver.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/card-list/CardList.tsx`.
- Runtime: `libs/cell-runtime-ui/src/primitives/card-list/CardList.types.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/card-list/index.ts`.
