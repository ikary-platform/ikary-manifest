...
Pagination provides the canonical implementation unit for the `pagination` contract in this repository. It converts validated presentation payloads into deterministic runtime rendering through the standard pipeline: schema, validator, adapter, resolver, and register. This LLD constrains implementation choices so code generation stays consistent with existing `cell-contract-presentation` and `cell-runtime-ui` patterns.

## 2. Scope

- Normalize and validate presentation shape defined by `PaginationPresentationSchema`.
- Expose a stable runtime contract for `Pagination` rendering.
...
- Own canonical fields such as `hideWhenSinglePage`, `visible`, `format`, `options`, `label`, `showFirst`.
...
- Presentation input: `PaginationPresentation` validated by the runtime validator in this folder.
...
- Primary schema fields consumed at runtime include `hideWhenSinglePage`, `visible`, `format`, `options`, `label`, `showFirst`, `showLast`, `showPrevious`.
...
- Contract schema: `libs/cell-contract-presentation/src/contract/pagination/PaginationPresentationSchema.ts`.
- LLD doc: `libs/cell-contract-presentation/src/contract/pagination/Pagination.LLD.md`.
- Runtime: `libs/cell-runtime-ui/src/primitives/pagination/Pagination.adapter.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/pagination/Pagination.example.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/pagination/Pagination.register.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/pagination/Pagination.resolver.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/pagination/Pagination.tsx`.
- Runtime: `libs/cell-runtime-ui/src/primitives/pagination/Pagination.types.ts`.
- Runtime: `libs/cell-runtime-ui/src/primitives/pagination/index.ts`.
